        ...
        switch (Robot.pathState) {
            ...
            case 2:
                if (!(timeShoot.seconds() < .7)) {
                    robot.stopShoot();
                    robot.follower.followPath(robot.follower.pathBuilder()
                            .addPath(new BezierCurve(GlobalPose.BLUE.BASIC_POSE_NEAR.scorePoseStart,
            ...
            case 3:
                if (!robot.follower.isBusy()) {
                    robot.follower.followPath(robot.follower.pathBuilder()
                            .addPath(new BezierLine(GlobalPose.BLUE.PICKUP_POSE.pickup2,
            ...
            case 5:
                if (!(timeShoot.seconds() < .7)) {
                    ...
                    robot.follower.setMaxPower(1);
                    robot.follower.followPath(robot.follower.pathBuilder()
                            .addPath(new BezierLine(GlobalPose.BLUE.BASIC_POSE_NEAR.scorePosePath,
            ...
            case 7:
                if (!robot.follower.isBusy()) {
                    ...
                    if (timeCase.seconds() > .2) {
                        robot.follower.followPath(robot.follower.pathBuilder()
                                .addPath(new BezierLine(GlobalPose.BLUE.RETAKE_POSE.pushLever,
                                        GlobalPose.BLUE.BASIC_POSE_NEAR.scorePosePath))
                                .setConstantHeadingInterpolation(Math.toRadians(225))
                                .build(), true);
            ...
            case 9:
                if (!(timeShoot.seconds() < .7)) {
                    robot.stopShoot();
                    robot.follower.followPath(robot.follower.pathBuilder()
                            .addPath(new BezierCurve(GlobalPose.BLUE.BASIC_POSE_NEAR.scorePosePath,
            ...
            case 10:
                if (!robot.follower.isBusy()) {
                    robot.follower.followPath(robot.follower.pathBuilder()
                            .addPath(new BezierLine(GlobalPose.BLUE.PICKUP_POSE.pickup1,
